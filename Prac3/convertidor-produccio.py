...
with open('fitxersDadesAnalisi/datos.json', 'r') as file:
    data = json.load(file)
...
structured_data = {}
for entry in data:
    country_code = entry['regio']
    best_match = find_best_match(country_code, geojson_country_codes)
    if best_match:
        structured_data[best_match] = {"contingut": entry['contingut']}
...
with open('updated_countries.geojson', 'w') as file:
    json.dump(geojson_data, file, indent=4, ensure_ascii=False)

print("GeoJSON actualizado y guardado en 'updated_countries.geojson'")
